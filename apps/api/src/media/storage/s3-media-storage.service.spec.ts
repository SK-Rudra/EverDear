...
function createStorage() {
  ...
  return new S3MediaStorageService(configService);
}
...
describe('S3MediaStorageService', () => {
  it('uploads a media object with its validated MIME type', async () => {
    const send = vi
    ...
    await storage.put({
      storageKey: 'letters/letter-id/attachment.png',
      buffer,
      mimeType: 'image/png',
    });
  ...
  it('returns a readable media stream', async () => {
    ...
    const result = await storage.get('letters/letter-id/attachment.png');
  ...
  it('rejects an unreadable response', async () => {
    ...
    await expect(
      storage.get('letters/letter-id/attachment.png'),
    ).rejects.toThrow('S3 returned an unreadable media object.');
  });
  ...
  it('deletes a media object', async () => {
    ...
    await storage.delete('letters/letter-id/attachment.png');
